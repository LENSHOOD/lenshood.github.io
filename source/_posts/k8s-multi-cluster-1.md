---
title: Kubernetes 多集群（上）：目的与核心问题
...
要更好的使用 K8s 多集群，多集群的管理必不可少，多集群管理面临的核心问题包括：

- 多集群部署模型
  - 控制面模型：多集群管理控制面所处的位置，控制面可以独占一个控制集群，也可以处于某个通用工作集群上。
  - 网络模型：集群间的交互需要底层的网络连通性，可以通过集群间的网关路由，也可以通过在多集群间建立虚拟扁平网络来实现连通性。
  - 服务注册与发现：某个集群内的应用，依赖另一个集群中的服务，应用需要获悉被依赖服务的访问地址。可以通过扩展单集群 DNS + Service 的概念到多集群，实现跨集群的服务注册与发现。
- 跨集群应用调度
  - 多集群的各种使用场景，本质上就是将应用部署在合适的集群上，高效实现这一切就需要自动化的动态调度。
  - 跨集群应用调度，实际上是基于某些调度策略，对应用属性与集群资源属性进行匹配，以期找到最佳的匹配关系。
- 应用模型扩展
  - 在多集群环境下的应用，其模型需要在规格与状态上进行扩展。规格包含了应用对集群偏好的限制与提示，状态则包含应用的调度历史，在不同集群上的副本数等等状态信息。
  - 通常多集群应用来源于单集群引用，因此多集群的应用模型扩展，最好确保与单集群应用模型的前向兼容性。
  - 需要考虑自定义资源在多集群环境下的扩展。为确保自定义资源在多集群中调度，需要考虑如何获取其规格和状态属性来满足调度策略。
- 集群即资源
  - 为了实现更高程度的自动化调度，集群本身也应作为一种资源，支持弹性扩缩。
  - 通过集群生命周期管理软件来实现自动化的创建、删除集群，并管理集群的加入、逐出。
  - 集群内的资源状态是集群扩缩的前提，这需要定义集群资源状态模型，以正确的汇报资源总量以及碎片情况。
