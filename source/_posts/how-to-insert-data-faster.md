...
考虑到事务提交频次与 insert 拼接消耗的平衡，经测试批次规模在 100~1000 之间效率最高。

另外，由于数据集和实际生成数据的比例在 1:10000 以上，因此无需每一次都进行随机选取，当内循环批次大小在几百至数千时数据离散程度不会发生明显变化。估将随机选取外循环，可以大幅降低计算次数。
...
最终，8 线程并发插入，插入速度同比又提升了 4 倍，并且 CPU 使用率达到 96%，IO Write 约 115 Mbps。
...
经过以上修改后的存储过程，整体 CPU 利用率下降了 2%，而 IO Write 则提升了 17%。
...
上文描述的存储过程优化，其场景简单、依赖少，很适合用来作为熟悉性能优化全过程的案例。在实践性能分析过程中，我们有以下几点思考：
