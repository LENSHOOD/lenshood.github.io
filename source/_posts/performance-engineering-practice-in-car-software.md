...
尝试落地持续性能优化的过程中，最困难的部分就是建立评估模型，因为评估模型代表性能目标。
...
最简单的观测系统可分为设备上的探针前端以及PC或服务器上的分析后端。由于直接运行在设备上，探针自身的开销不容忽视，因此通常性能探针都包含细粒度的配置能力，在研发或测试阶段，打开全部或大部分开关，而在生产运行阶段只打开部分开销低的信息收集功能。
