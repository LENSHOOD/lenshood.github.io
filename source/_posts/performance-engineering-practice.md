...
如图所示，为了深入的了解服务或模块的性能变化，一般采用性能剖析的手段，在函数级别进行数据采样。采样剖析可以生成函数调用栈、函数总耗时 / On CPU 耗时、函数内存消耗、堆内存分析等报告，通过它们来寻找劣化代码块。通过对各类性能剖析工具进行封装，可以固化用例、简化操作，并实现工具自动 attach 到 Pod 或实例，剖析完成后数据自动上报，生成分析结果图。
